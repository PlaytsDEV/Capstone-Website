...
import { CustomDateRangeModal } from "../../pages/analyticsTabShared";
import { buildRangeLabel } from "../../pages/reportCommon";
...
export default function AnalyticsToolbar({
  ...
  const currentRangeDays = useMemo(() => {
    const match = String(range?.value || "").match(/^(\d+)d$/i);
    return match ? parseInt(match[1], 10) : 30;
  }, [range?.value]);

  const computedRangeOptions = useMemo(() => {
    if (!range?.options) return [];
    const list = [...range.options];
    const hasCurrent = list.some((opt) =>
      typeof opt === "string" ? opt === range.value : opt.value === range.value
    );
    if (!hasCurrent && range.value) {
      list.push({
        value: range.value,
        label: buildRangeLabel(range.value),
      });
    }
    if (range.allowCustom !== false) {
      const hasCustom = list.some((opt) =>
        typeof opt === "string" ? opt === "__custom__" : opt.value === "__custom__"
      );
      if (!hasCustom) {
        list.push({
          value: "__custom__",
    ...
    return list;
  }, [range?.options, range?.value, range?.allowCustom]);
  ...
  const handleRangeChange = (e) => {
    ...
    if (selected === "__custom__") {
      ...
      range.onChange?.(selected);
    }
  };

  const handleCustomApply = (days) => {
    range.onChange?.(`${days}d`);
  };
  ...
  return (
    <>
        ...
        <div className={`analytics-toolbar__controls ${controlsClassName}`}>
          {range ? (
            <label className={`analytics-toolbar__field ${controlClassName}`}>
              ...
              >
                {renderOptions(computedRangeOptions)}
              </select>
      ...
      {range && range.allowCustom !== false && (
        <CustomDateRangeModal
          isOpen={isCustomModalOpen}
          onClose={() => setIsCustomModalOpen(false)}
          onApply={handleCustomApply}
          initialDays={currentRangeDays}
        />
      )}
    </>
  );
