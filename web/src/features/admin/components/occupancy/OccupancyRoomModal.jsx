...
export default function OccupancyRoomModal({ room, loadingDetails, onClose }) {
 ...
 const getBedTone = (kind) => {
   switch (kind) {
     case "occupied":
       return {
         card: "bg-blue-50/50 border-slate-200 dark:bg-blue-950/20 dark:border-slate-800",
         icon: "text-blue-600 dark:text-blue-400",
         accent: "text-info-dark",
       };
     case "reserved":
       return {
         card: "bg-amber-50/50 border-slate-200 dark:bg-amber-950/20 dark:border-slate-800",
         icon: "text-amber-600 dark:text-amber-400",
         accent: "text-warning-dark",
       };
     case "locked":
       return {
         card: "bg-muted/50 border-slate-200 dark:border-slate-800",
         icon: "text-muted-foreground",
         accent: "text-card-foreground",
       };
     case "maintenance":
       return {
         card: "bg-orange-50/50 border-slate-200 dark:bg-orange-950/20 dark:border-slate-800",
         icon: "text-orange-600 dark:text-orange-400",
         accent: "text-orange-700 dark:text-orange-400",
       };
     default:
       return {
         card: "bg-emerald-50/50 border-slate-200 dark:bg-emerald-950/20 dark:border-slate-800",
 ...
 <div key={idx} className={`flex items-start gap-3 p-3 rounded-lg border ${occupiedTone.card}`}>
 ...
 {(() => {
                              ...
                              let badgeStyle = "bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700 font-medium";
                              let textLabel = `${days} days left`;
                              if (days < 0) {
                                badgeStyle = "bg-rose-50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-300 border-rose-200 dark:border-rose-800 font-bold";
                                textLabel = `Overdue (${Math.abs(days)}d)`;
                              } else if (days === 0) {
                                badgeStyle = "bg-rose-50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-300 border-rose-200 dark:border-rose-800 font-bold";
                                textLabel = "Vacant Today";
                              } else if (days <= 7) {
                                badgeStyle = "bg-rose-50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-300 border-rose-200 dark:border-rose-800 font-bold";
                                textLabel = `${days} ${days === 1 ? "day" : "days"} left`;
                              } else if (days <= 30) {
                                badgeStyle = "bg-amber-50 dark:bg-amber-950/40 text-amber-800 dark:text-amber-300 border-amber-200 dark:border-amber-800 font-semibold";
                                textLabel = `${days} days left`;
                              } else if (days <= 90) {
                                badgeStyle = "bg-sky-50 dark:bg-sky-950/40 text-sky-700 dark:text-sky-300 border-sky-200 dark:border-sky-800 font-medium";
                                textLabel = `${days} days left`;
                              }
                              return (
                                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] border ${badgeStyle}`}>
                                  {textLabel}
                                </span>
                              );
                            })()}
 ...
 <div key={idx} className={`flex items-start gap-3 p-3 rounded-lg border ${reservedTone.card}`}>
 ...
 <div key={idx} className={`flex items-start gap-3 p-3 rounded-lg border ${lockedTone.card}`}>
 ...
 <div key={idx} className={`flex items-start gap-3 p-3 rounded-lg border ${maintenanceTone.card}`}>
 ...
 <div key={idx} className={`flex items-start gap-3 p-3 rounded-lg border ${availableTone.card}`}>
