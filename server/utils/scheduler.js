...
async function reconcileMissingContractGeneration() {
  try {
    ...
    }).select("_id reservationId status createdBy updatedBy").lean();
    ...
    for (const contract of stuckContracts) {
      ...
      const reservation = await Reservation.findById(contract.reservationId)
        .select("status initialPaymentStatus paymentStatus confirmedMoveInDate moveInDate")
        .lean();
      if (!reservation) continue;

      const movedIn = reservation.status === "moveIn";
      const depositSettled =
        reservation.initialPaymentStatus === "paid" || reservation.paymentStatus === "paid_in_full";
      // Neither trigger condition has actually happened yet — nothing is
      // stuck, this Contract is just waiting on the tenant/admin as designed.
      if (!movedIn && !depositSettled) continue;
