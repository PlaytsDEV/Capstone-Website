...
// {to, templateKey, variables, idempotencyKey} and hands it to the single
// authoritative sendLilycrestEmail() router in lilycrestEmailService.js,
// which itself decides Resend-Template-vs-inline-HTML. These tests mock
// that one boundary and assert each function maps its caller-provided data
// onto the correct template key and variables, and that a provider failure
// is never reported as a successful send.

const sendLilycrestEmail = jest.fn();

await jest.unstable_mockModule("../services/email/lilycrestEmailService.js", () => ({
  sendLilycrestEmail,
}));
...
beforeEach(() => {
  jest.clearAllMocks();
  sendLilycrestEmail.mockResolvedValue({ success: true, provider: "resend", messageId: "msg_1" });
});
...
describe("sendEmailVerificationLinkEmail", () => {
  test("maps to the EMAIL_VERIFICATION template with the Firebase-generated link as a variable", async () => {
    ...
    expect(sendLilycrestEmail).toHaveBeenCalledWith(
    ...
    expect(typeof sendLilycrestEmail.mock.calls[0][0].idempotencyKey).toBe("string");
  });

  test("a provider rejection is propagated as failure, never reported as success", async () => {
    sendLilycrestEmail.mockResolvedValue({ success: false, code: "EMAIL_PROVIDER_AUTH_REJECTED" });
...
describe("sendPasswordResetLinkEmail", () => {
  ...
  test("verification and password-reset idempotency keys never collide for the same address+link shape", async () => {
    ...
    await email.sendPasswordResetLinkEmail({ to: "tenant@example.test", name: "Jose", resetLink: link });
    const resetKey = sendLilycrestEmail.mock.calls[0][0].idempotencyKey;

    sendLilycrestEmail.mockClear();
    await email.sendEmailVerificationLinkEmail({ to: "tenant@example.test", name: "Jose", verificationLink: link });
    const verifyKey = sendLilycrestEmail.mock.calls[0][0].idempotencyKey;

    expect(resetKey).not.toBe(verifyKey);
  });

  test("a provider rejection is propagated as failure, never reported as success", async () => {
    sendLilycrestEmail.mockResolvedValue({ success: false, code: "EMAIL_DELIVERY_FAILED" });
...
describe("sendLoginOtpEmail", () => {
  ...
  test("a provider rejection is propagated as failure, never a false requiresOtp-style success", async () => {
    sendLilycrestEmail.mockResolvedValue({ success: false, code: "EMAIL_PROVIDER_NOT_CONFIGURED" });
...
describe("transactional emails map to their own distinct templates with only real values", () => {
  ...
  test("sendInquiryResponseEmail rejects an obviously invalid recipient before ever calling the provider", async () => {
    ...
    expect(result.success).toBe(false);
    expect(sendLilycrestEmail).not.toHaveBeenCalled();
  });
});
