...
await jest.unstable_mockModule("../utils/notificationService.js", () => ({
  notify: { general: jest.fn(), adminReply: jest.fn() },
}));
...
describe("startConversation — contract context", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    // profileImage short-circuits resolveTenantProfileImage() before it
    // needs to touch Reservation/User again — not the behavior under test.
    userFindById.mockReturnValue({
      lean: async () => ({ _id: TENANT_ID, branch: "gil-puyat", profileImage: "https://example.test/avatar.png" }),
    });
