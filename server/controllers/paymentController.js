  ...
  try {
    ...
    if (isPaid) {
      ...
      if (bill) {
        ...
        // settlePaymongoBill owns the Bill ledger write and the derived
        // Reservation projection. Do not independently promote an initial
        // payment to paid here: a gateway session can be paid even when the
        // internal ledger write was not applied and needs reconciliation.
        ...
        if (!settlement.applied) {
          const internalBill = getVisibleBillSnapshot(bill);
          logger.info(
            {
              billId: bill._id,
              paymentReference,
              reason: settlement.reason,
              internalBillStatus: internalBill.status,
              internalRemainingAmount: internalBill.remainingAmount,
            },
            "Bill polling settlement did not apply a new ledger entry",
          );
        } else {
