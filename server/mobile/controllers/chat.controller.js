...
try {
  ({ emitToChatAdmins: _emitToChatAdmins } = require('../utils/socket'));
} catch (_) {}
function safeEmitChatAdmins(branch, event, payload) {
  if (_emitToChatAdmins) _emitToChatAdmins(branch, event, payload);
}
...
const ADMIN_ROLES = new Set(['admin', 'owner', 'branch_admin', 'superadmin' /* legacy */]);
...
async function notifyAdminsOfTenantMessage(db, conversation) {
  try {
    const admins = await db.collection('users').find({
      ...
      $or: [
        { role: { $in: ['owner', 'branch_admin', 'superadmin' /* legacy */] } },
...
async function startConversation(req, res) {
  try {
    ...
    if (conversation) {
      ...
      // Notify web admin panel about the new conversation in real-time.
      safeEmitChatAdmins(conversation.branch, 'chat:conversation-updated', serializeConversation(conversation));
    }
      ...
      db.collection(CHAT_MESSAGES).updateMany(
        {
          ...
          senderRole: { $in: ['admin', 'owner', 'superadmin' /* legacy */] },
          readAt: null,
        },
        { $set: { readAt: now } },
      ),
