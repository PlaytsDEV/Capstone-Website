...
} from "./documentStorageAuthorization.service.js";

// Unit tests for the MOB-P0-01 storage-authorization invariant itself: a
// document's downloadUrl, storagePath, and the configured bucket must all
// identify the exact same object under the authenticated tenant's own
// upload prefix. Ported from the standalone LilyCrest-Mobile backend's
// backend/tests/documentStorageAuthorization.test.js — same cases, same
// reasons, proven again against this canonical copy of the module.
...
describe("authorizeTenantStorageObject", () => {
  test("accepts a storagePath that matches the download URL and the tenant own prefix", () => {
    ...
    expect(result.objectPath).toBe(path);
  });
  ...
  test("rejects when the submitted storagePath does not match the decoded URL object path (own valid URL paired with a foreign path)", () => {
    ...
    const result = authorizeTenantStorageObject({
      downloadUrl: downloadUrlFor(own),
      storagePath: foreign,
      userId: TENANT_A,
      configuredBucket: BUCKET,
    });
  ...
  test("rejects when the URL genuinely points elsewhere even though the submitted storagePath looks like the tenant's own object", () => {
    const own = ownPath(TENANT_A);
    const foreign = ownPath(TENANT_B);
    const result = authorizeTenantStorageObject({
      downloadUrl: downloadUrlFor(foreign),
      storagePath: own,
      userId: TENANT_A,
      configuredBucket: BUCKET,
    });
    expect(result.authorized).toBe(false);
    expect(result.reason).toBe("url_path_mismatch");
  });
  ...
  test("rejects a URL pointing at a different bucket than configured", () => {
    ...
    expect(result.authorized).toBe(false);
    expect(result.reason).toBe("wrong_bucket");
  });
  ...
  test("rejects an unrecognized/non-canonical download URL host", () => {
    ...
    expect(result.authorized).toBe(false);
    expect(result.reason).toBe("unrecognized_url");
    expect(decodeFirebaseDownloadUrl(`https://evil.example.com/${path}`)).toBeNull();
  });
  ...
  test("rejects encoded (%2e%2e / %2f) traversal tricks inside the URL object segment", () => {
    ...
    expect(decodeFirebaseDownloadUrl(url)).toBeNull();
  });
