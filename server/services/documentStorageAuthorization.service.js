/**
 * ============================================================================
 * MOBILE DOCUMENT STORAGE AUTHORIZATION (P0)
 * ============================================================================
 *
 * Canonical storage-authorization invariant for tenant-uploaded documents,
 * ported from the standalone LilyCrest-Mobile backend's
 * backend/services/documentStorageAuthorization.service.js (MOB-P0-01).
 *
 * POST /api/m/users/documents (services/mobileUserDocumentService.js,
 * uploadUserDocument()) previously trusted a client-supplied `storagePath`
 * outright: any authenticated tenant could register another tenant's real
 * object path (or any path in the bucket) under their own document metadata,
 * and the later GET .../content and DELETE endpoints would use privileged
 * Firebase Admin access against that unverified path.
 *
 ...
 *
 * The tenant prefix is keyed on the mobile session's user_id (not the Mongo
 * _id) to match the real object layout POST /api/m/upload/firebase-storage
 * writes to (routes/mobileUploadRoutes.js: `[folder, tenantId, entityId,
 * ...]`, where tenantId = req.mobileTenant.user_id, and the mobile app's
 * my-documents.jsx passes folder: 'tenant-documents' for this exact flow)
 * — that route is itself safe because it derives tenantId server-side and
 * ignores any client-supplied value; this module exists for the separate
 * /api/m/users/documents metadata-registration endpoint, which accepts a
 * storagePath from the client directly and has no such guarantee on its own.
 */
...
// download URL shape (the exact format POST /api/m/upload/firebase-storage
...
export function decodeFirebaseDownloadUrl(url) {
  let parsed;
  try {
...
export function tenantDocumentPrefix(userId) {
...
export function authorizeTenantStorageObject({ downloadUrl, storagePath, userId, configuredBucket } = {}) {
